...
import com.nabhatek.catalogservice.exceptions.FileEmptyException;
import com.nabhatek.catalogservice.exceptions.FileUploadException;
import com.nabhatek.catalogservice.service.FileService;
...
public class FileUploadController {
    ...
    public ResponseEntity<?> uploadFile(@RequestParam("file") MultipartFile multipartFile) throws FileEmptyException, FileUploadException, IOException {
        ...
        if (isValidFile && allowedFileExtensions.contains(FilenameUtils.getExtension(multipartFile.getOriginalFilename()))){
            ...
            apiResponse.setMessage("file uploaded successfully. File unique name =>" + fileName);
            ...
            APIResponse apiResponse = new APIResponse();
            apiResponse.setMessage("Invalid File. File extension or File name is not supported");
    ...
    public ResponseEntity<?> downloadFile(@RequestParam("fileName")  @NotBlank @NotNull String fileName) throws FileDownloadException, IOException {
        ...
        if (response != null){
            ...
            APIResponse apiResponse = new APIResponse();
            apiResponse.setMessage("File could not be downloaded");
            apiResponse.setSuccessful(false);
            apiResponse.setStatusCode(400);
            /*
            APIResponse apiResponse = APIResponse.builder()
    ...
    public ResponseEntity<?> delete(@RequestParam("fileName") @NotBlank @NotNull String fileName){
        ...
        if (isDeleted){
            APIResponse apiResponse = new APIResponse();
            apiResponse.setMessage("file deleted!");
            apiResponse.setSuccessful(true);
            apiResponse.setStatusCode(200);
//            APIResponse apiResponse = APIResponse.builder().message("file deleted!").statusCode(200).build();
            ...
            APIResponse apiResponse = new APIResponse();
            apiResponse.setMessage("file does not exist");
            apiResponse.setSuccessful(false);
